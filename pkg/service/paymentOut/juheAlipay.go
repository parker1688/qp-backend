...
func init() {
	channelPayOut["AliPay"] = &JuHeAliPayPayOutGateway{}
}

// JuHeAliPayPayOutGateway
// @Description:
type JuHeAliPayPayOutGateway struct {
}

func (T JuHeAliPayPayOutGateway) PreWithdraw(param PreOtherPaymentOutParam) PrePaymentChannelOutResp {
	return PrePaymentChannelOutResp{
		Code: 200,
	}
}

func (T JuHeAliPayPayOutGateway) ChannelWithdraw(param OtherPaymentOutParam) PaymentChannelOutResp {
	...
	body := map[string]string{
		...
		"extra":     param.UserId,
		"notifyUrl": conf.AliPayVirtualNotifyURLOut,
	...
	global.G_LOG.Infof("JuHeAliPayPayOutGateway url:%v request:%v status:%v resp: %v err:%v", url, signStr, resp.StatusCode(), resp.String(), err)
	if err != nil {
...
func (T JuHeAliPayPayOutGateway) ChannelCallBack(param CallBackPaymentOutParam) CallBackPaymentOutResp {
	conf := global.CONFIG.Payment.YinRunPay
	...
	signM := strings.ToUpper(tool.MD5([]byte(signStr + "&key=" + conf.Md5Key)))
	if sign != signM {
		global.G_LOG.Errorf("用户(%s)代付订单(%s)回调签名失败 raw=%s", userId, body["mchOrderNo"], signStr)
...
func (T JuHeAliPayPayOutGateway) FindOrderStatus(merchantOrderId string) FindOrderPaymentOutResp {
	conf := global.CONFIG.Payment.YinRunPay
...
func (T JuHeAliPayPayOutGateway) ChannelCallBackGetOrderSn(param CallBackPaymentOutParam) CallBackPaymentOutResp {
	/*extraData := gjson.Get(param.Raw, "extra").String() //系统订单号
	_, orderSn := GetOrderExtraData(extraData)
	if len(orderSn) == 0 {
		global.G_LOG.Errorf("[ChannelCallBackGetOrderSn] JuHeAliPayPayOutGateway Unmarshal extra failed: extra=%s", extraData)
	...
	if err != nil {
		global.G_LOG.Errorf("[ChannelCallBackGetOrderSn] JuHeAliPayPayOutGateway Unmarshal extra failed: raw=%s", param.Raw)
	...
	if v, ok := reqData["mchOrderNo"]; ok {
		...
		global.G_LOG.Errorf("[ChannelCallBackGetOrderSn] JuHeAliPayPayOutGateway mchOrderNo null: raw=%s", param.Raw)
	}
