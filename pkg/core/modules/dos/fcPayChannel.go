...
type FcPayChannel struct {
	...
	InputAmountDisplay     int                `gorm:"input_amount_display" json:"input_amount_display"`                                  // 输入金额是否显示  0 不显示  1 显示
	...
	Strategy               string             `gorm:"strategy" json:"strategy" form:"strategy" uri:"strategy" `                          // 支付限单
}

func (FcPayChannel) TableName() string {
	return "fc_pay_channel"
}

type FcPayChannelExt struct {
	FcPayChannel
	Merchant FcMerchant `json:"merchant" gorm:"foreignkey:MerchantCode;references:MerchantCode"`
}

type FcPayChannelResp struct {
	FcPayChannel
	MerchantName string `json:"merchant_name"`
}

type FcPayChannelStrategyField struct {
	Time int64 `json:"time"` // 支付限单时间(分)
	Num  int64 `json:"num"`  // 支付限单数量
}
