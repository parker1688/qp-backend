...
OUTDIR=/data/qpbg/web
BASENAME=web
LOGPATH=/var/log/qpbg

app=$BASENAME
conf=$OUTDIR/conf.dev.yaml
pidfile=$OUTDIR/$BASENAME.pid
logfile=$LOGPATH/web.log
...
function start() {
    ...
    nohup  ./$BASENAME --config=$conf >$logfile 2>&1 &
    sleep 1
...
function stop() {
    pid=`ps -ef|grep $BASENAME|grep -v grep|awk '{print $2}'|xargs kill -9`
    #pid=`cat $pidfile`
    kill $pid
    rm -f $pidfile
