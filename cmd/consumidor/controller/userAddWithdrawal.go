...
func init() {
	kafkaInitFun = append(kafkaInitFun, WithdrawalConsumer)
}

// UserInviteConsumer
//
//	@Description: 充值消费
func WithdrawalConsumer(ctx context.Context) {
	...
	kafkaOpt.CreateConsumerGroup(strings.Split(conf.Addr, ","), &kafkaMQ.Kafka{
		...
		Topic:   []string{channelData.Kakfa_Topic_User_Withdrawal},
		Handler: func(message *sarama.ConsumerMessage) error {
			err := WithdrawalAccumulation(message.Value)
			if err != nil {
...
// 存款累计报表
func WithdrawalAccumulation(s []byte) error {
	defer func() {
		if r := recover(); r != nil {
			const size = 64 << 10
			buf := make([]byte, size)
	...
	var m channelData.UserWithdrawalMessage
	err := tool.JsonUnmarshal(s, &m)
	if err != nil {
		global.G_LOG.Infof("消费提款 累计总存款 解码json err:%s", err.Error())
		return err
	}
	...
	key := fmt.Sprintf("WithdrawalAccumulation::%s", m.OrderSn)
	//加分布式锁
	if global.G_REDIS.SetNX(context.Background(), key, "true", time.Duration(30)*time.Minute).Val() {
		...
		if len(report.Id) == 0 {
			...
			global.G_DB.Model(&dos.FcUserReport{}).Where("user_id=?", m.UserId).Updates(map[string]interface{}{
				"recharge_amount": gorm.Expr("withdrawal_amount + ?", m.WithdrawalAmount),
				"recharge_count":  gorm.Expr("withdrawal_count + ?", 1),
			})
		}
	}
