...
func GetQRCodeGoogleUrl(name,secret,title string)string{

	optStr := fmt.Sprintf("otpauth://totp/%s?",name)

	otpParams := url.Values{
		"secret":{secret},
	}
	if title != ""{
		otpParams.Set("issuer",title)
	}

	optStr = optStr + otpParams.Encode()
	...
	return "https://chart.googleapis.com/chart?chs=200x200&chld=M|0&cht=qr&" + urlParams.Encode()
}